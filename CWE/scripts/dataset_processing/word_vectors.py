...
class Word_vectors(object):
    ...
    def create_vocabulary(self, dataset):
        ...
        word_to_id = {k:(v+INDEX_FROM) for k,v in word_index_1.items()}
        word_to_id["<PAD>"] = 0
        word_to_id["<START>"] = 1
        word_to_id["<UNK>"] = 2
        idx2word = {value:key for key,value in word_to_id.items()}

        return word_index_1, idx2word, tokenizer
    ...
    def create_word_vectors(self, dataset=None):
        try:
            ...
            word_index, idx2word, tokenizer = self.create_vocabulary(dataset)
            ...
            with open("assets/input_dataset/"+self.config["asset_name"]+"/word_index.pickle", "wb") as handle:
                pickle.dump(word_index, handle)
            with open("assets/input_dataset/"+self.config["asset_name"]+"/word_vectors.npy", "wb") as handle:
                np.save(handle, word_vectors)

            return idx2word, tokenizer
